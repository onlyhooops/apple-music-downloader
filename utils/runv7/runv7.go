...
func downloadChunk(wg *sync.WaitGroup, errChan chan error, fileUrl string, header http.Header, tempFile *os.File, chunkIndex int, start, end int64) {
	...
	if err != nil {
		errChan <- fmt.Errorf("chunk %d: failed to read body: %w", chunkIndex, err)
		return
	}

	_, err = tempFile.WriteAt(body, start)
	if err != nil {
		errChan <- fmt.Errorf("chunk %d: failed to write to temp file: %w", chunkIndex, err)
		return
	}
}
