...
func ensureSafePath(basePath, artistDir, albumDir, fileName string, trackNum int) (string, string, string) {
	...
	for {
		...
		minStem := fmt.Sprintf("%02d", trackNum)
		if len(stem) > len(minStem) {
			canShorten := len(stem) - len(minStem)
			shortenAmount := overage
			if shortenAmount > canShorten {
				shortenAmount = canShorten
			}
			stem = truncate(stem, shortenAmount)
			fileName = stem + ext
			continue
		}

		if len(albumDir) > 1 { // 至少保留一个字符
			canShorten := len(albumDir) - 1
			shortenAmount := overage
			if shortenAmount > canShorten {
				shortenAmount = canShorten
			}
...
func downloadTrackSilently(track structs.TrackData, meta *structs.AutoGenerated, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec, covPath string) error {
	...
	_, _, finalFilename := ensureSafePath(basePath, artistDir, albumDir, filenameWithExt, trackNum)
...
func rip(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string) error {
	...
	trackTotalForSelect := len(meta.Data[0].Relationships.Tracks.Data)
	longestTrackNum := 0
	for i := 0; i < trackTotalForSelect; i++ {
	...
	finalArtistDir, finalAlbumDir, _ := ensureSafePath(basePath, sanitizedSingerFoldername, sanitizedAlbumFolder, longestFilename, longestTrackNum)
