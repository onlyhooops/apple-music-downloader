...
func downloadTrackWithFallback(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, workingAccounts []structs.Account, initialAccountIndex int, statusIndex int, updateStatus func(index int, status string, sColor func(a ...interface{}) string), progressChan chan runv12.ProgressUpdate) error {
...
func downloadTrackSilently(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, account *structs.Account, progressChan chan runv12.ProgressUpdate) error {
	...
	if needDlAacLc {
		...
		err = runv12.Run(track.ID, trackM3u8Url, trackPath, account, Config, progressChan)
		if err != nil {
...
func renderUI(done <-chan struct{}) {
	ticker := time.NewTicker(300 * time.Millisecond) // 调整为300ms，减少闪烁
	defer ticker.Stop()
	uiMutex.Lock()
...
func printUI() {
	...
	builder.WriteString(fmt.Sprintf("\033[%dA", len(trackStatuses)))
...
func rip(albumId string, storefront string, urlArg_i string, urlRaw string) error {
	...
	for i, trackNum := range selected {
		...
		go func(trackIndexInMeta int, statusIndex int) {
			...
			red := color.New(color.FgRed).SprintFunc()
			yellow := color.New(color.FgYellow).SprintFunc()
			progressChan := make(chan runv12.ProgressUpdate, 10)
			go func() {
	...
	for {
		for albumNum, urlRaw := range os.Args {
			...
			if strings.Contains(urlRaw, "/music-video/") {
				...
				mvInfo, err := getMVInfoFromAdam(albumId, accountForMV, storefront)
				if err != nil {
				...
				if Config.ArtistFolderFormat != "" {
					artistFolder = strings.NewReplacer(
						"{UrlArtistName}", LimitString(mvInfo.Data[0].Attributes.ArtistName),
						"{ArtistName}", LimitString(mvInfo.Data[0].Attributes.ArtistName),
						"{ArtistId}", "",
					).Replace(Config.ArtistFolderFormat)
				}
				sanitizedArtistFolder := forbiddenNames.ReplaceAllString(artistFolder, "_")

				err = mvDownloader(albumId, Config.AlacSaveFolder, sanitizedArtistFolder, "", storefront, nil, accountForMV)
		...
		//fmt.Scanln() // 此行代码会暂停程序直到用户按下回车。注释掉后，程序将在出错后自动重试。
		fmt.Println("开始重试...")
		sharedLock.Lock()
		counter = structs.Counter{}
		sharedLock.Unlock()
...
func mvDownloader(adamID string, baseSaveDir, artistDir, albumDir string, storefront string, meta *structs.AutoGenerated, account *structs.Account) error {
	...
	finalArtistDir, finalAlbumDir, finalFilename := ensureSafePath(baseSaveDir, artistDir, albumDir, filenameWithExt)
	...
	if true {
		...
		covPath, err = writeCover(finalAlbumFolder, baseThumbName, thumbURL)
		if err == nil {
			tags = append(tags, fmt.Sprintf("cover=%s", covPath))
		}
	}
