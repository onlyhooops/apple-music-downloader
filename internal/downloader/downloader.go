...
func getAudioStreamInfo(filePath string) (*audioStreamInfo, error) {
	cmd := exec.Command("ffprobe", "-v", "error", "-select_streams", "a:0",
		"-show_entries", "stream=sample_fmt,sample_rate,channel_layout",
	...
	if len(lines) < 3 {
		return nil, fmt.Errorf("ffprobe output is incomplete: %s", out.String())
	}

	return &audioStreamInfo{
		SampleFmt:     lines[0],
		SampleRate:    lines[1],
		ChannelLayout: lines[2],
	}, nil
}

func checkAndFixTrack(trackPath string, statusIndex int, track structs.TrackData) (bool, error) {
	ui.UpdateStatus(statusIndex, "正在检测...", color.New(color.FgCyan).SprintFunc())

	checkCmd := exec.Command("ffmpeg", "-v", "error", "-i", trackPath, "-f", "null", "-")
	var stderr bytes.Buffer
	...
	ui.UpdateStatus(statusIndex, "发现坏帧, 正在修复...", color.New(color.FgYellow).SprintFunc())
	finalFixedPath := strings.TrimSuffix(trackPath, filepath.Ext(trackPath)) + "-fix.m4a"
	...
	if runtime.GOOS == "darwin" {
		if _, err := exec.LookPath("afconvert"); err == nil {
			afconvertCmd := exec.Command("afconvert", "-f", "m4af", "-d", "alac", trackPath, finalFixedPath)
			if output, err := afconvertCmd.CombinedOutput(); err == nil {
				fixed = true
			} else {
				os.Remove(finalFixedPath)
				fixErr = fmt.Errorf("afconvert fix failed: %w\nOutput: %s", err, string(output))
			}
		}
	}
	...
	if !fixed {
		...
		if durationInMillis <= 0 {
			return false, fmt.Errorf("cannot fix track, duration metadata is missing")
		}

		tempCleanPath := strings.TrimSuffix(trackPath, filepath.Ext(trackPath)) + "-temp-clean.m4a"
		defer os.Remove(tempCleanPath)

		extractCmd := exec.Command("ffmpeg", "-y", "-err_detect", "ignore_err", "-fflags", "+discardcorrupt", "-i", trackPath, "-c", "copy", tempCleanPath)
		if output, err := extractCmd.CombinedOutput(); err != nil {
			return false, fmt.Errorf("ffmpeg fix step 1 (extract) failed: %w\nOutput: %s", err, string(output))
		}

		info, err := getAudioStreamInfo(tempCleanPath)
		if err != nil {
			return false, fmt.Errorf("ffmpeg fix step 2 (probe) failed: %w", err)
		}

		durationInSeconds := fmt.Sprintf("%.3f", float64(durationInMillis)/1000.0)
		filterComplex := fmt.Sprintf(
			"aevalsrc=0:d=%s:s=%s:channel_layout=%s[canvas];[0:a]asetpts=PTS-STARTPTS[audio];[canvas][audio]amix=inputs=2:duration=first[mixout]",
			durationInSeconds, info.SampleRate, info.ChannelLayout,
		)

		remuxCmd := exec.Command("ffmpeg", "-y", "-i", tempCleanPath,
			"-filter_complex", filterComplex,
			"-c:a", "alac",
			"-sample_fmt", info.SampleFmt,
			"-c:v", "copy",
			"-map", "0:v?",
			"-map", "[mixout]",
			finalFixedPath)

		var remuxStderr bytes.Buffer
		remuxCmd.Stderr = &remuxStderr
		if err := remuxCmd.Run(); err != nil {
			return false, fmt.Errorf("ffmpeg fix step 3 (remux) failed: %w\nOutput: %s", err, remuxStderr.String())
		}
	}

	if err := os.Remove(trackPath); err != nil {
		return true, fmt.Errorf("failed to remove original corrupted file: %w", err)
	}
	if err := os.Rename(finalFixedPath, trackPath); err != nil {
		return true, fmt.Errorf("failed to rename fixed file: %w", err)
	}

	return true, fixErr
}

func downloadTrackWithFallback(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, workingAccounts []structs.Account, initialAccountIndex int, statusIndex int, updateStatus func(index int, status string, sColor func(a ...interface{}) string), progressChan chan runv14.ProgressUpdate) (string, error) {
...
func Rip(albumId string, storefront string, urlArg_i string, urlRaw string) error {
	...
	for i, trackNum := range selected {
		...
		go func(trackIndexInMeta int, statusIndex int) {
			...
			trackPath, err := downloadTrackWithFallback(trackData, meta, albumId, storefront, baseSaveFolder, Codec, covPath, workingAccounts, statusIndex, statusIndex, ui.UpdateStatus, progressChan)
			...
			if !isAAC {
				var fixErr error
				wasCorrupt, fixErr = checkAndFixTrack(trackPath, statusIndex, trackData)
				if fixErr != nil {
					ui.UpdateStatus(statusIndex, fmt.Sprintf("修复失败: %v", fixErr), color.New(color.FgRed).SprintFunc())
					core.Counter.Error++
					core.SharedLock.Unlock()
					return
				}
			...
			err = metadata.WriteMP4Tags(trackPath, finalLrc, meta, trackIndexInMeta, len(meta.Data[0].Relationships.Tracks.Data))
			if err != nil {
				ui.UpdateStatus(statusIndex, fmt.Sprintf("标签写入失败: %v", err), color.New(color.FgYellow).SprintFunc())
			} else {
				if wasCorrupt {
					ui.UpdateStatus(statusIndex, "修复完成", color.New(color.FgRed).SprintFunc())
				} else {
