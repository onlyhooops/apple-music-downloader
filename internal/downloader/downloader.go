...
func checkAndFixTrack(trackPath string, statusIndex int, track structs.TrackData, trackNum int, urlRaw string, workingAccounts []structs.Account) (bool, error) {
	...
	ui.UpdateStatus(statusIndex, "发现坏帧, 准备修复...", color.New(color.FgRed).SprintFunc())

	if err := os.Remove(trackPath); err != nil {
		return false, fmt.Errorf("删除损坏文件失败: %w", err)
	}

	parsedURL, err := url.Parse(urlRaw)
	if err != nil {
		return false, fmt.Errorf("无法解析专辑URL: %w", err)
	}
	pathParts := strings.Split(strings.Trim(parsedURL.Path, "/"), "/")
	storefront := ""
	if len(pathParts) > 1 {
		storefront = pathParts[0]
	}

	var prioritizedAccounts []structs.Account
	for _, acc := range workingAccounts {
		if acc.Storefront == storefront {
			prioritizedAccounts = append(prioritizedAccounts, acc)
		}
	}
	for _, acc := range workingAccounts {
		if acc.Storefront != storefront {
	...
	var lastFixError error
	for _, account := range prioritizedAccounts {
		statusMsg := fmt.Sprintf("尝试使用 [%s] 修复...", account.Name)
		ui.UpdateStatus(statusIndex, statusMsg, color.New(color.FgYellow).SprintFunc())

		cmd := exec.Command("go", "run", "fix.go",
			"--select",
			"--track-number", strconv.Itoa(trackNum),
			"--account-name", account.Name,
			urlRaw)

		output, err := cmd.CombinedOutput()
		if err == nil {
			ui.UpdateStatus(statusIndex, "修复成功!", color.New(color.FgRed).SprintFunc())
			return true, nil
		}

		lastFixError = fmt.Errorf("账户 [%s] 修复失败: %v\nOutput: %s", account.Name, err, string(output))
	}

	return false, fmt.Errorf("所有账户均修复失败: %w", lastFixError)
}
...
func Rip(albumId string, storefront string, urlArg_i string, urlRaw string) error {
	...
	for i, trackNum := range selected {
		...
		go func(trackIndexInMeta int, statusIndex int) {
			...
			if !isAAC {
				var fixErr error
				wasCorrupt, fixErr = checkAndFixTrack(trackPath, statusIndex, trackData, trackIndexInMeta, urlRaw, workingAccounts)
				if fixErr != nil {
