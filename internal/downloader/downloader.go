...
func downloadTrackSilently(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, finalSaveFolder, Codec, covPath string, account *structs.Account, progressChan chan runv14.ProgressUpdate) (string, error) {
	...
	if manifest.Attributes.ExtendedAssetUrls.EnhancedHls == "" {
		...
		if core.Dl_aac && (*core.Aac_type == "binaural" || *core.Aac_type == "downmix") {
	...
	} else if core.Dl_aac && *core.Aac_type == "binaural" {
		Tag_string = utils.FormatQualityTag("Aac Binaural")
	} else if core.Dl_aac && *core.Aac_type == "downmix" {
...
func Rip(albumId string, storefront string, urlArg_i string, urlRaw string, notifier *progress.ProgressNotifier) error {
	...
	// 验证下载完整性：在转移缓存文件之前，检查是否真的下载了完整的音乐文件
	if usingCache {
		// 检查目标文件夹中是否有足够数量的音乐文件
		expectedTrackCount := len(meta.Data[0].Relationships.Tracks.Data)
		actualMusicFiles := 0

		err := filepath.Walk(finalAlbumFolder, func(path string, info os.FileInfo, err error) error {
			if err != nil {
				return nil
			}
			if !info.IsDir() && (strings.HasSuffix(path, ".m4a") || strings.HasSuffix(path, ".mp3")) {
				actualMusicFiles++
			}
			return nil
		})

		if err == nil && actualMusicFiles > 0 {
			minRequiredFiles := expectedTrackCount / 2 // 至少需要一半的歌曲文件
			if actualMusicFiles >= minRequiredFiles {
				logger.Info("✅ 验证通过：发现 %d 个音乐文件（预期 %d 个）", actualMusicFiles, expectedTrackCount)
			} else {
				logger.Warn("⚠️ 验证警告：仅发现 %d 个音乐文件（预期 %d 个），可能下载不完整", actualMusicFiles, expectedTrackCount)
				// 不阻止转移，但发出警告
			}
		}
	}
	...
	if usingCache {
		...
		// 检查是否有足够的音乐文件
		minRequiredFiles := expectedTrackCount / 2 // 至少需要一半的歌曲文件
		if err == nil && actualMusicFiles >= minRequiredFiles {
			logger.Info("✅ 缓存验证通过：发现 %d 个音乐文件（预期 %d 个）", actualMusicFiles, expectedTrackCount)

			// 递归扫描整个缓存hash目录，查找所有需要转移的文件
			hasFilesToMove := false
			scanErr := filepath.Walk(cacheHashDir, func(path string, info os.FileInfo, err error) error {
				if err != nil {
			...
			if scanErr := utils.CleanupCacheDirectory(cacheHashDir); scanErr != nil {
				logger.Warn("清理缓存目录警告: %v", scanErr)
			}

			// 设置下载成功标志
			downloadSuccess = true
		} else {
			// 缓存验证失败：音乐文件太少，清理缓存并记录错误
			logger.Warn("❌ 缓存验证失败：仅发现 %d 个音乐文件（预期至少 %d 个），清理缓存", actualMusicFiles, minRequiredFiles)
			if err := utils.CleanupCacheDirectory(cacheHashDir); err != nil {
				logger.Warn("清理缓存目录警告: %v", err)
			}
			// 设置标志避免延迟清理再次清理
			downloadSuccess = true
			return fmt.Errorf("下载验证失败：音乐文件数量不足（%d/%d）", actualMusicFiles, minRequiredFiles)
		}
	} else {
		// 没有使用缓存，设置成功标志
		downloadSuccess = true
