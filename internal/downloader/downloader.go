...
func downloadTrackSilently(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, account *structs.Account, progressChan chan runv14.ProgressUpdate) (string, error) {
	...
	if core.Dl_atmos {
		qualityTag = utils.FormatQualityTag("Dolby Atmos")
	} else if needDlAacLc {
		qualityTag = utils.FormatQualityTag("Aac 256")
	} else {
		// 根据音频特性判断是否为 Hi-Res Lossless
		if utils.Contains(track.Attributes.AudioTraits, "hi-res-lossless") {
			qualityTag = utils.FormatQualityTag("Hi-Res Lossless")
		} else if utils.Contains(track.Attributes.AudioTraits, "lossless") {
			qualityTag = utils.FormatQualityTag("Alac")
		} else {
			qualityTag = utils.FormatQualityTag("Alac")
		}
	}
	...
	// 添加音质标签（如果配置中使用了 {Tag} 变量）
	if qualityTag != "" {
		stringsToJoin = append(stringsToJoin, qualityTag)
	}
...
func Rip(albumId string, storefront string, urlArg_i string, urlRaw string) error {
	...
	if core.Dl_atmos {
		...
		// 无损格式：检查是否为 Hi-Res Lossless
		if len(meta.Data[0].Relationships.Tracks.Data) > 0 {
			firstTrack := meta.Data[0].Relationships.Tracks.Data[0]
			if utils.Contains(firstTrack.Attributes.AudioTraits, "hi-res-lossless") {
...
func MvDownloader(adamID string, baseSaveDir, artistDir, albumDir string, storefront string, meta *structs.AutoGenerated, account *structs.Account) (string, error) {
	...
	// 如果配置了独立的 MV 保存路径，则使用它；否则使用传入的 baseSaveDir
	if core.Config.MvSaveFolder != "" {
		baseSaveDir = core.Config.MvSaveFolder
	}

	// Emby 命名标准: {VideoName (Year)}/{VideoName (Year)}.mp4
	// 歌手名称已经在父文件夹中，无需重复
	...
	// 使用 artistDir 作为 MV 保存文件夹下的子文件夹进行组织
	...
	if true {
		...
		baseThumbName := core.ForbiddenNames.ReplaceAllString(mvFileName, "_") + "_thumbnail"
